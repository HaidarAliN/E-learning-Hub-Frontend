...
const useStyles = makeStyles((theme) => ({
  ...
  emptyState: {
    ...
    [theme.breakpoints.up("md")]: {
      marginTop: `calc(${
        window.innerHeight / 3 - 0.05 * window.innerHeight
      }px)`,
      textAlign: "center",
...
export default function Notifications() {
  ...
  const getDAta = async () => {
    const response = await axios.get(
      `${BASE_API_URL}/api/student/get-notifications`,
      {
        headers: {
  ...
  const handleRead = async (id) => {
    const response = await axios.post(
      `${BASE_API_URL}/api/student/notifications/mark-read`,
      {
        notification_id: id,
      },
      {
  ...
  const NavigateToCourse = async (course_id) => {
    const response2 = await axios.get(
      `${BASE_API_URL}/api/student/get-course-name-by-id/${course_id}`,
      {
        headers: {
          Authorization: `Bearer ${access_token}`,
        },
